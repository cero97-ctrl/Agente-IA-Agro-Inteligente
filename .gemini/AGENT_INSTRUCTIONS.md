...
- **Aislamiento:** Uso de contenedor Docker con `build-essential` para validación de lógica y compilación.
...
1.  **Generar `env_diagnostic.py`:** Si el entorno es desconocido o ha cambiado, genera un script que recolecte:
    ...
    - **PACKAGES:** Versiones de librerías críticas (ej. `numpy`, `serial`).
