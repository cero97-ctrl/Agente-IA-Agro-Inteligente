# Sistema de Agro-Inteligencia y Monitoreo de Cultivos IA

## Descripción
Este proyecto es una plataforma de **Agricultura de Precisión y Monitoreo de Cultivos** impulsada por Inteligencia Artificial. Utilizando la arquitectura de 3 capas del *Gemini Agent Framework*, el sistema actúa como un asistente agrónomo avanzado capaz de:

- **Diagnosticar enfermedades y plagas** mediante análisis visual con ESP32-CAM.
- **Generar reportes de cultivo** basados en investigación agronómica y datos visuales.
- **Interactuar por voz y texto** vía Telegram para asistencia en campo a agricultores.
- **Gestionar memoria a largo plazo** para recordar historiales de parcelas, riegos y tratamientos.
...
Para iniciar el agente agrónomo y conectarlo a Telegram:
```bash
...
- **`/rol [agronomo/productor]`**: Cambia tu perfil de usuario para acceder a distintas herramientas.
- **`/modo [default/serio/pirata...]`**: Cambia la personalidad del agente.
...
### Gestión de Cultivos (Rol Agrónomo)
- **`/nuevo_cultivo [Nombre/Variedad]`**: Registra una nueva parcela o planta (ID automático).
- **`/cultivos`**: Lista todos los cultivos activos bajo monitoreo.
- **`/monitorear [ID]`**: Ver estado actual y últimos registros del cultivo.
- **`/foto [flash]`**: Captura y **analiza** una imagen de la planta (detecta plagas/enfermedades).
- **`/captura [flash]`**: Toma una foto rápida sin análisis (más veloz).
- **`/vigilancia [on/off]`**: Activa el modo centinela (toma fotos periódicas para timelapse o detección).
- **`/flash [on/off]`**: Enciende o apaga la linterna de la cámara manualmente.
- **`/simular_plaga [ID]`**: Simula una alerta de plaga para pruebas del sistema.
- **`/tratamiento [ID]`**: Registra una acción correctiva (riego, poda, fumigación).

### Documentación
En la carpeta `docs/` encontrarás los manuales de usuario (`manual_agronomo.tex`) listos para compilar a PDF.
Para generarlos (requiere tener LaTeX instalado):
```bash
pdflatex -output-directory=docs docs/manual_agronomo.tex
```
