...
def set_my_commands():
    """Configura los comandos del bot en la interfaz de Telegram."""
    if not TOKEN:
        print(json.dumps({"status": "error", "message": "Falta TELEGRAM_BOT_TOKEN"}))
        sys.exit(1)
    ...
    commands = [
        {"command": "ayuda", "description": "Ver herramientas disponibles"},
        {"command": "monitorear", "description": "Estado de sensores y cultivos"},
        {"command": "foto", "description": "Capturar y analizar imagen (IA)"},
        {"command": "reporte", "description": "Generar informe agronómico"},
        {"command": "investigar", "description": "Búsqueda técnica en internet"},
        {"command": "cultivos", "description": "Listar cultivos activos"},
        {"command": "status", "description": "Estado del sistema"},
        {"command": "rol", "description": "Cambiar perfil (Agrónomo/Productor)"}
    ]
    
    try:
        response = requests.post(url, json={"commands": commands}, timeout=10)
        response.raise_for_status()
        print(json.dumps({"status": "success", "message": "Menú de comandos activado."}))
    except Exception as e:
        print(json.dumps({"status": "error", "message": str(e)}))
        sys.exit(1)
...
def main():
    ...
    parser.add_argument("--action", choices=["send", "check", "get-id", "download", "send-photo", "send-document", "send-voice", "set-commands"], required=True, help="Acción a realizar.")
